# =============================================================================
# app/models/quotation_item.py
# -----------------------------------------------------------------------------
# ORM model for a single priced line item within a Quotation. Optionally
# references the source RFQItem and the accepted SupplierQuote so that the
# full procurement traceability chain (RFQ → SupplierQuote → QuotationItem)
...
class QuotationItem(AuditMixin, Base):
    """A single priced line item within a Quotation.

    Table: ``quotation_items``

    Key relationships:
    - ``quotation``      — parent Quotation (cascade delete).
    - ``rfq_item``       — optional source RFQ line item for traceability.
    - ``supplier_quote`` — optional accepted supplier quote used to price this line.

    The ``margin_percent`` property provides a quick gross-margin calculation
    when ``cost_price`` is recorded.
    """
    ...
    # ── Traceability (procurement chain) ─────────────────────────────────────
    ...
    def calculate_line_total(self) -> float:
        """Compute and persist ``line_total`` from quantity, price, discount, and tax.

        Formula: ``qty × unit_price × (1 - discount%) × (1 + tax%)``

        Returns:
            The computed line total, also stored in ``self.line_total``.
        """
    ...
    def margin_percent(self) -> Optional[float]:
        """Gross margin as a percentage of unit_price, if cost_price is set.

        Returns:
            Margin percentage rounded to 2 decimal places, or ``None`` if
            ``cost_price`` is not recorded.
        """
        if self.cost_price and self.unit_price:
            return round(
                (float(self.unit_price) - float(self.cost_price))
                / float(self.unit_price) * 100,
                2,
            )
        return None
